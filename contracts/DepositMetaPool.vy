...
def __init__(_pool: address, _token: address):
    ...
    self.pool = _pool
    self.token = _token
    _base_pool: address = MetaPool(_pool).base_pool()
    self.base_pool = _base_pool

    for i in range(N_COINS):
        coin: address = MetaPool(_pool).coins(convert(i, uint256))
        self.coins[i] = coin
    ...
    for i in range(BASE_N_COINS):
        coin: address = BasePool(_base_pool).coins(convert(i, uint256))
...
def add_liquidity(amounts: uint256[N_ALL_COINS], min_mint_amount: uint256) -> uint256:
    ...
    if deposit_base:
        BasePool(self.base_pool).add_liquidity(base_amounts, 0)
        meta_amounts[MAX_COIN] = ERC20(self.coins[MAX_COIN]).balanceOf(self)

    # Deposit to the meta pool
    MetaPool(self.pool).add_liquidity(meta_amounts, min_mint_amount)
...
def remove_liquidity(_amount: uint256, min_amounts: uint256[N_ALL_COINS]) -> uint256[N_ALL_COINS]:
    ...
    MetaPool(self.pool).remove_liquidity(_amount, min_amounts_meta)

    # Withdraw from base
    _base_amount: uint256 = ERC20(self.coins[MAX_COIN]).balanceOf(self)
    for i in range(BASE_N_COINS):
        min_amounts_base[i] = min_amounts[MAX_COIN+i]
    BasePool(self.base_pool).remove_liquidity(_base_amount, min_amounts_base)
...
def remove_liquidity_one_coin(_token_amount: uint256, i: int128, _min_amount: uint256) -> uint256:
    ...
    if i < MAX_COIN:
        ...
        MetaPool(self.pool).remove_liquidity_one_coin(_token_amount, i, _min_amount)
    else:
        coin = self.base_coins[i - MAX_COIN]
        # Withdraw a base pool coin
        MetaPool(self.pool).remove_liquidity_one_coin(_token_amount, MAX_COIN, 0)
        BasePool(self.base_pool).remove_liquidity_one_coin(
...
def remove_liquidity_imbalance(amounts: uint256[N_ALL_COINS], max_burn_amount: uint256) -> uint256:
    ...
    fee: uint256 = BasePool(_base_pool).fee() * BASE_N_COINS / (4 * (BASE_N_COINS - 1))
    ...
    if withdraw_base:
        amounts_meta[MAX_COIN] = BasePool(self.base_pool).calc_token_amount(amounts_base, False)
        amounts_meta[MAX_COIN] += amounts_meta[MAX_COIN] * fee / FEE_DENOMINATOR + 1

    # Remove liquidity and deposit leftovers back
    MetaPool(_meta_pool).remove_liquidity_imbalance(amounts_meta, max_burn_amount)
    if withdraw_base:
        BasePool(_base_pool).remove_liquidity_imbalance(amounts_base, amounts_meta[MAX_COIN])
        leftover_amounts[MAX_COIN] = ERC20(_meta_coins[MAX_COIN]).balanceOf(self)
        if leftover_amounts[MAX_COIN] > 0:
            MetaPool(_meta_pool).add_liquidity(leftover_amounts, 0)
...
def calc_withdraw_one_coin(_token_amount: uint256, i: int128) -> uint256:
    ...
    """
    if i < MAX_COIN:
        return MetaPool(self.pool).calc_withdraw_one_coin(_token_amount, i)
    else:
        _base_tokens: uint256 = MetaPool(self.pool).calc_withdraw_one_coin(_token_amount, MAX_COIN)
        return BasePool(self.base_pool).calc_withdraw_one_coin(_base_tokens, i-MAX_COIN)
...
def calc_token_amount(amounts: uint256[N_ALL_COINS], is_deposit: bool) -> uint256:
    ...
    _base_tokens: uint256 = BasePool(self.base_pool).calc_token_amount(base_amounts, is_deposit)
    meta_amounts[MAX_COIN] = _base_tokens

    return MetaPool(self.pool).calc_token_amount(meta_amounts, is_deposit)
