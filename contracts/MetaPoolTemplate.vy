...
MAX_PROTOCOL_FEE: constant(uint256) = 10 * 10 ** 9
...
protocol_fee: public(uint256)  # protocol_fee * 1e10
...
future_protocol_fee: public(uint256)
...
def __init__(
    ...
    self.base_virtual_price = RosePool(_base_pool).get_virtual_price()
    self.base_cache_updated = block.timestamp
    for i in range(BASE_N_COINS):
        _base_coin: address = RosePool(_base_pool).coins(convert(i, uint256))
...
def _vp_rate() -> uint256:
    if block.timestamp > self.base_cache_updated + BASE_CACHE_EXPIRES:
        vprice: uint256 = RosePool(self.base_pool).get_virtual_price()
...
def _vp_rate_ro() -> uint256:
    if block.timestamp > self.base_cache_updated + BASE_CACHE_EXPIRES:
        return RosePool(self.base_pool).get_virtual_price()
    else:
        return self.base_virtual_price
...
def add_liquidity(amounts: uint256[N_COINS], min_mint_amount: uint256) -> uint256:
    ...
    _protocol_fee: uint256 = self.protocol_fee
    ...
    if token_supply > 0:
        ...
        for i in range(N_COINS):
            ...
            self.balances[i] = new_balances[i] - (fees[i] * _protocol_fee / FEE_DENOMINATOR)
...
def get_dy_underlying(i: int128, j: int128, dx: uint256) -> uint256:
    ...
    if base_i < 0:
        ...
        if base_j < 0:
            ...
            x = RosePool(_base_pool).calc_token_amount(base_inputs, True) * vp_rate / PRECISION
            # Accounting for deposit/withdraw fees approximately
            x -= x * RosePool(_base_pool).fee() / (2 * FEE_DENOMINATOR)
            # Adding number of pool tokens
            x += xp[MAX_COIN]
        else:
            # If both are from the base pool
            return RosePool(_base_pool).get_dy(base_i, base_j, dx)
    ...
    if base_j < 0:
        ...
        dy = RosePool(_base_pool).calc_withdraw_one_coin(dy * PRECISION / vp_rate, base_j)
...
def exchange(i: int128, j: int128, dx: uint256, min_dy: uint256) -> uint256:
    ...
    dy_protocol_fee: uint256 = dy_fee * self.protocol_fee / FEE_DENOMINATOR
    ...
    self.balances[j] = old_balances[j] - dy - dy_protocol_fee
...
def exchange_underlying(i: int128, j: int128, dx: uint256, min_dy: uint256) -> uint256:
    ...
    if base_i < 0 or base_j < 0:
        ...
        if base_i < 0:
            ...
            RosePool(_base_pool).add_liquidity(base_inputs, 0)
        ...
        dy_protocol_fee: uint256 = dy_fee * self.protocol_fee / FEE_DENOMINATOR
        dy_protocol_fee = dy_protocol_fee * PRECISION / rates[meta_j]

        # Change balances exactly in same way as we change actual ERC20 coin amounts
        self.balances[meta_i] = old_balances[meta_i] + dx_w_fee
        # When rounding errors happen, we undercharge protocol fee in favor of LP
        self.balances[meta_j] = old_balances[meta_j] - dy - dy_protocol_fee

        # Withdraw from the base pool if needed
        if base_j >= 0:
            out_amount: uint256 = ERC20(output_coin).balanceOf(self)
            RosePool(_base_pool).remove_liquidity_one_coin(dy, base_j, 0)
        ...
        RosePool(_base_pool).exchange(base_i, base_j, dx_w_fee, min_dy)
...
def remove_liquidity_one_coin(_token_amount: uint256, i: int128, _min_amount: uint256) -> uint256:
    ...
    self.balances[i] -= (dy + dy_fee * self.protocol_fee / FEE_DENOMINATOR)
...
@external
def commit_new_fee(new_fee: uint256, new_protocol_fee: uint256):
    assert msg.sender == self.owner  # dev: only owner
    assert self.admin_actions_deadline == 0  # dev: active action
    assert new_fee <= MAX_FEE  # dev: fee exceeds maximum
    assert new_protocol_fee <= MAX_PROTOCOL_FEE  # dev: protocol fee exceeds maximum

    _deadline: uint256 = block.timestamp + ADMIN_ACTIONS_DELAY
    self.admin_actions_deadline = _deadline
    self.future_fee = new_fee
    self.future_protocol_fee = new_protocol_fee

    log CommitNewFee(_deadline, new_fee, new_protocol_fee)
...
def apply_new_fee():
    ...
    _protocol_fee: uint256 = self.future_protocol_fee
    self.fee = _fee
    self.protocol_fee = _protocol_fee

    log NewFee(_fee, _protocol_fee)
...
@external
def transfer_fee_receiver(_receiver: address):
    assert msg.sender == self.owner  # dev: only owner

    self.fee_receiver = _receiver

    log TransferFeeReceiver(_receiver)


@view
@external
def protocol_balances(i: uint256) -> uint256:
...
def withdraw_protocol_fees():
    ...
    for i in range(N_COINS):
        ...
        if value > 0:
            assert ERC20(c).transfer(self.fee_receiver, value)
