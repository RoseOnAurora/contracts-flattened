...
def get_virtual_price() -> uint256:
    ...
    token_supply: uint256 = LPToken(self.lp_token).totalSupply()
...
def calc_token_amount(amounts: uint256[N_COINS], deposit: bool) -> uint256:
    ...
    token_amount: uint256 = LPToken(self.lp_token).totalSupply()
    diff: uint256 = 0
    if deposit:
        diff = D1 - D0
    else:
...
def add_liquidity(amounts: uint256[N_COINS], min_mint_amount: uint256):
    ...
    # Mint pool tokens
    LPToken(self.lp_token).mint(msg.sender, mint_amount)
...
def remove_liquidity(_amount: uint256, min_amounts: uint256[N_COINS]):
    ...
    LPToken(self.lp_token).burnFrom(msg.sender, _amount)  # dev: insufficient funds
...
def remove_liquidity_imbalance(amounts: uint256[N_COINS], max_burn_amount: uint256):
    ...
    LPToken(self.lp_token).burnFrom(msg.sender, token_amount)  # dev: insufficient funds
    for i in range(N_COINS):
        if amounts[i] != 0:
    ...
    for i in range(N_COINS):
        ...
        if value > 0:
            ...
            _response: Bytes[32] = raw_call(
                ...
                concat(
                    method_id("transfer(address,uint256)"),
                    convert(self.fee_receiver, bytes32),
                    convert(value, bytes32),
                ),
                max_outsize=32,
            )  # dev: failed transfer
